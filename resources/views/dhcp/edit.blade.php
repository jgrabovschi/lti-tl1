...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
    ...
    @enderror
    <form method="POST" action="{{ route('updateDhcp', ['id' => $dhcp->{'.id'}]) }}" class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
        @csrf
        @method('PATCH')
        <div class="mb-4">
            <label for="name" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Name</label>
            <input type="text" id="name" name="name" value="{{ old('name', $dhcp->name) }}" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
        </div>
        @error('name')
        ...
        <div class="mb-4">
            ...
            @foreach ($interfaces as $interface)
                <option value="{{$interface}}" {{ old('interface', $interface) == $dhcp->interface ? 'selected' : '' }} >{{ $interface }}</option>
            @endforeach
            </select>
        </div>
        @error('interface')
        ...
        <div class="mb-4">
            ...
            @foreach ($pools as $pool)
                <option value="{{$pool}}" {{ old('pool', $pool) == $dhcp->{'address-pool'} ? 'selected' : '' }} >{{ $pool }}</option>
            @endforeach
            </select>
        </div>
        @error('pool')
