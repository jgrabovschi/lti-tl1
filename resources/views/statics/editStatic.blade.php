...
<div class="relative overflow-x-auto shadow-md sm:rounded-lg mt-4">
    <form method="POST" action="{{ route('updateStatic', ['id' => $id]) }}" class="p-4 bg-white dark:bg-gray-800 rounded-lg shadow">
        @csrf
        @method('PATCH')
        <div class="mb-4">
            ...
            <input type="text" id="dst-address" name="dst-address" value="{{ old('name', json_decode($data)->{'dst-address'})}}" placeholder="Ex: 192.168.1.1/24" required class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
        </div>
        
        <div class="mb-4">
            <label for="gateway" class="block text-sm font-medium text-gray-700 dark:text-gray-300">Gateway </label>
            <input type="text" id="gateway" name="gateway" required  value="{{ old('name', json_decode($data)->gateway)}}" class="mt-1 block w-full px-3 py-2 border border-gray-300 rounded-md shadow-sm focus:outline-none focus:ring-blue-500 focus:border-blue-500 dark:bg-gray-700 dark:border-gray-600 dark:text-white">
        </div>
